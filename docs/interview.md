...
mobile kpi lab은 Galaxy S23에서 stable diffusion 계열 모델을 실행시키고 latency, quality, thermal 등을 정량 측정한 프로젝트입니다.
...
6. **전력 측정 한계** — BatteryManager 기반 시스템 전체 소비전력으로, NPU 단독 전력 분리 불가. 다만 충전기 분리·airplane mode·brightness 고정 등 변인을 통제해 모든 실험을 동일 조건에서 측정했으므로, 절대값보다 실험 간 상대 차이(burst vs balanced −22%, precision 변경 −19% 등) 해석에 활용했다.
