...
| 파일 | 크기 | 생성 방법 | 상태 | .bin | profile (Inf / Peak Mem) |
|---|---|---|---|---|---|
| `vae_encoder_fp32.onnx` | 130MB | `scripts/sd/export_sd_to_onnx.py` | ✅ | `j57dwqdl5` (63MB) | `jpev13e05` (200ms / 1-79MB) |
| `vae_encoder_int8_qdq.onnx` | 33MB | `scripts/sd/quant_runpod.py` | ✅ CosSim 0.985 | `jp0rq73ep` (37MB) | `jgz79ko6p` (**51ms** / 0-38MB) |
| `vae_encoder_qai_int8.onnx` + `.data` | 0.4MB + 131MB | QAI Hub quantize (`jgjlm961p`) | ✅ CosSim 0.984 | `j5q2kj245` (30MB) | `j5w9vn2jp` (**53ms** / 1-38MB) |

### Text Encoder (`onnx/text_encoder_*`)

| 파일 | 크기 | 생성 방법 | 상태 | .bin | profile (Inf / Peak Mem) |
|---|---|---|---|---|---|
| `text_encoder_fp32.onnx` | 470MB | `scripts/sd/export_sd_to_onnx.py` | ✅ | `jpydw9d7p` (218MB) | `jg941ejv5` (**6.6ms** / 1-3MB) |
| `text_encoder_qai_int8.onnx` + `.data` | 0.6MB + 470MB | QAI Hub quantize (`jpev1q085`) | ❌ CosSim -0.05 | `jglkzjk8p` (102MB) | — |

> INT8 양자화 품질 완전 파괴 (CosSim -0.05). FP32/FP16 필수.
...
| 파일 | 크기 | 생성 방법 | 상태 | .bin | profile (Inf / Peak Mem) |
|---|---|---|---|---|---|
| `unet_fp32.onnx` + `.data` | 0.8MB + 3.2GB | `scripts/sd/export_sd_to_onnx.py` | ✅ opset 18 | `jgov0yex5` (1.5GB) | `jp18lxylg` (335ms / 0-1794MB) |
| `unet_mixed_pr.onnx` + `.data` | 1.9MB + 821MB | `scripts/sd/quant_runpod.py --op-types Conv MatMul Gemm` | ✅ CosSim 0.994 | `jgkynzmyp` (1.1GB) | `jgdv9lelg` (**265ms** / 0-1282MB) |
...
| 파일 | 크기 | 생성 방법 | 상태 | .bin | profile (Inf / Peak Mem) |
|---|---|---|---|---|---|
| `vae_decoder_fp32.onnx` | 189MB | `scripts/sd/export_sd_to_onnx.py` | ✅ | `jpx1jw11g` (91MB) | `j57dw3lr5` (448ms / 0-114MB) |
| `vae_decoder_int8_qdq.onnx` | 48MB | `scripts/sd/quant_runpod.py` | ✅ CosSim 0.960 | `jp879v085` (54MB) | `jp4wo0dlg` (**104ms** / 0-58MB) |
| `vae_decoder_qai_int8.onnx` + `.data` | 0.5MB + 189MB | QAI Hub quantize (`j57dwm1n5`) | ✅ CosSim 0.968 | `j561jk10p` (46MB) | `jpx1j269g` (**110ms** / 1-68MB) |
...
| 파일 | 크기 | 생성 방법 | 상태 | .bin | profile (Inf / Peak Mem) |
|---|---|---|---|---|---|
| `yolov8n-seg_fp32.onnx` | 13MB | `scripts/yolo/export_yolo_seg.py` | ✅ | `j5mz2jqqp` FP16 (6.3MB) | `j5mz2y6qp` (**6ms** / 15-22MB) |
| `yolov8n-seg_int8_qdq_noh.onnx` | 6.9MB | `scripts/yolo/export_yolo_seg.py` | ✅ mAP@50 -1.8% | `j561jkn0p` (3.9MB) | `jgney8mmg` (**4ms** / 15-17MB) |
...
|---|---|---|---|---|---|
| vae_encoder | Custom QDQ | 0.985 | 0.153 | 28.6dB | Marginal |
| vae_encoder | QAI Hub W8A8 | 0.984 | 0.154 | 28.6dB | Marginal |
| text_encoder | QAI Hub W8A8 | **-0.048** | 1.295 | 28.1dB | **Poor** |
| vae_decoder | Custom QDQ | 0.960 | 0.184 | 17.8dB | Marginal |
| vae_decoder | QAI Hub W8A8 | 0.968 | 0.153 | 19.4dB | Marginal |
| unet | Mixed Precision (Conv/MatMul INT8) | **0.994** | 0.106 | 32.3dB | **Good** |
