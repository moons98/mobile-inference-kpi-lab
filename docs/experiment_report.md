...
**결론**: balanced mode는 warm latency +13~14% 손해, 전력 −27% 절감. Cold E2E 기준으로는 증가 폭이 더 작아(+5~9%) 실서비스 영향이 제한적이다. background 추론 또는 배터리 절약 시나리오에서 유효한 선택지.
...
### 6.1 Cloud latency 대비

기준: Galaxy S26 Ultra cloud 실행 latency ~10s (표 1 참조). 본 실험 해상도는 512×512.
> 참고: cloud의 1024×1024 생성과 직접 비교 시, on-device 512 생성 + SR upscale 파이프라인이 현실적 (섹션 7.1 참조).

**표 6.1.** Cloud 대비 Latency 비교

| Config | E2E warm (ms) | Cold E2E (ms) |
|---|---|---|
| SD A4 burst (s20) | 11,286 | 16,178 |
| SD C1 balanced (s20) | 12,789 | 17,624 |
| LCM A6 burst (s4) | 1,693 | 7,999 |
| LCM C2 balanced (s4) | 1,931 | 8,360 |

SD v1.5는 warm 11.3s로 cloud 기준(~10s)을 이미 초과하며, cold 16.2s는 1.6배에 달한다. balanced 모드에서는 더 악화(12.8s / 17.6s)되어 **on-device 추론이 어렵다**.

LCM-LoRA는 warm 1.7s로 cloud 대비 ×0.17, cold 8.0s에서도 cloud 이하를 유지한다. balanced 모드(1.9s / 8.4s)에서도 cloud 기준을 충족하여 **burst/balanced 모두 feasible**하다.

### 6.2 KPI별 판정

**표 6.2.** KPI별 수치 비교

| KPI | SD v1.5 A4 | LCM A6 |
|---|---|---|
| Latency warm | 11.3s | 1.7s |
| Latency cold | 16.2s | 8.0s |
| Memory (NPU+App) | 1,709MB | 2,320MB |
| Thermal end | 46.5°C | 37.5°C |
| CLIP Score | 35.97 | 30.91 |

- **Latency**: cloud 기준 ~10s 대비 SD는 warm부터 초과(11.3s), LCM은 cold 포함해도 이하(8.0s). LCM만 feasible.
- **Memory**: SD 1,709MB로 12GB RAM 기기에서 여유 있음. LCM은 2,320MB로 더 높지만(UNet fp16) crash-free 운용 가능 수준.
- **Thermal**: SD는 46.5°C로 OEM 개입 구간(42~45°C)에 근접하여 주의 필요. LCM은 37.5°C로 안정적.
- **Quality**: SD CLIP 35.97(양호), LCM 30.91(보통, 27~33 구간). LCM은 프롬프트 충실도가 낮으나 실용 가능 수준.
...
2. **LCM-LoRA가 on-device feasibility의 열쇠** — SD v1.5는 warm 11.3s로 cloud 기준 초과. LCM-LoRA는 warm 1.7s / cold 8.0s로 cold start 포함해도 cloud 이하이며, Snapdragon 8 Gen 2에서 실사용 가능한 on-device 이미지 생성이 가능하다.

3. **Mixed Precision이 품질 유지하며 KPI 개선** — UNet MIXED_PR + VAE W8A8(A4)은 E2E −22%, Mem −29%, 전력 −19% 절감, CLIP 동등. LCM vae w8a8(A6)은 VAEDec −73%, LPIPS 0.03(사실상 동일).

4. **Step 증가는 품질 이득 없음** — SD s20→s50, LCM s4→s8 모두 CLIP ±2 이내. UNet per-step latency는 step 수와 무관하게 일정하므로 E2E는 step에 선형 비례.

5. **Balanced mode: latency +14%, 전력 −27%** — SD/LCM 동일 비율. NPU clock scaling이 모델 무관하게 일관 적용. Background 추론 시 유효한 선택지.

6. **LCM의 메모리 역설** — LCM UNet이 fp16이라 SD UNet MIXED_PR보다 NPU 메모리가 크다(1,793MB vs 1,151MB). Latency/전력에서 유리하지만 메모리에서는 불리.

7. **연속 추론에서도 thermal throttling 없음** — 10회 연속 burst 시 LCM +2.1%, SD +0.07% drift. Feasibility 판정은 연속 사용에서도 유지.

### 6.4 권장 운영점

**표 6.3.** 권장 운영점

| Use Case | 권장 Config | E2E (warm) | Cold E2E | Power AvgInfer |
|---|---|---|---|---|
| Foreground 빠른 생성 | LCM s4 vae w8a8 burst (A6) | 1.7s | 8.0s | 3,999mW |
| Background / 배터리 절약 | LCM s4 vae w8a8 balanced (C2) | 1.9s | 8.4s | 3,127mW |
| SD 품질 필요 시 | SD s20 mixed_pr+w8a8 burst (A4) | 11.3s | 16.2s | 4,525mW |
...
**LCM-LoRA의 trade-off와 시사점:**

본 실험에서 LCM-LoRA는 SD v1.5 대비 latency를 6.7× 단축(11.3s→1.7s)했으나, CLIP Score가 30.91로 SD의 35.97 대비 낮아 프롬프트 충실도에서 trade-off가 존재한다. 이는 LCM이 few-step distillation으로 속도를 얻는 대신 prior의 표현력 일부를 희생하기 때문이다.

따라서 multi-service 확장 시:
- **속도 우선 기능**(스티커 생성, 빠른 프리뷰): LCM UNet 사용
- **품질 우선 기능**(배경화면, 고품질 편집): SD UNet 사용
- 두 UNet 모두 동일 VAE Decoder를 공유하므로, LoRA weight swap만으로 전환 가능
...
**결론**: 모바일 diffusion 서비스는 **"Shared UNet backbone + lightweight task-specific adapters"** 구조를 기본으로 두고, 품질 요구가 높은 일부 기능만 별도 모델(LoRA/fine-tuned UNet)을 고려하는 전략이 합리적이다.
